...
const ParallaxBackground: React.FC = () => {
  ...
  const macOSWindows = Array.from({ length: 4 }, (_, i) => ({
    id: i,
    width: 140 + i * 25,
    height: 90 + i * 20,
    x: 15 + i * 20,
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 2 }}>
      ...
      {/* Floating Code Snippets with Enhanced Styling */}
      {codeSnippets.map((snippet, index) => (
        <motion.div
          key={snippet}
          className="absolute text-xs font-mono text-white/15 select-none px-2 py-1 rounded bg-white/5 backdrop-blur-sm border border-white/10"
          style={{
            left: `${(index * 12 + 8) % 85}%`,
            top: `${(index * 18 + 12) % 75}%`,
          }}
      ...
      {macOSWindows.map((window, i) => (
        <motion.div
          key={`window-${i}`}
          className="absolute bg-white/8 backdrop-blur-md rounded-xl border border-white/15 shadow-2xl"
          style={{
            width: window.width,
        ...
        >
          ...
          <div className="flex items-center space-x-2 p-3">
            <motion.div 
              className="w-3 h-3 bg-red-500/60 rounded-full backdrop-blur-sm"
              whileHover={{ scale: 1.2, backgroundColor: "rgba(239, 68, 68, 0.8)" }}
            />
            <motion.div 
              className="w-3 h-3 bg-yellow-500/60 rounded-full backdrop-blur-sm"
              whileHover={{ scale: 1.2, backgroundColor: "rgba(245, 158, 11, 0.8)" }}
            />
            <motion.div 
              className="w-3 h-3 bg-green-500/60 rounded-full backdrop-blur-sm"
